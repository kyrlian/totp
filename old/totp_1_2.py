...
TITLE = "Typing Of The Python 1.2 (c) Kyrlian 2008"
...
def ShowScoresMode(screen):
    ...
    while 1:
        screen.fill(Color("black"))
        screen.blit(titleren, (SCREEN.centerx-titleren.get_width()/2, SCREEN.height/nbs*1-titleren.get_height()/2))
        font = pygame.font.Font(TTFONT, MESSAGEFONTSIZE)
        for i in range(nbl):
            ren = font.render(scorelist[i][0:-1], 1, Color("white"))
            screen.blit(ren, (SCREEN.centerx-ren.get_width()/2, SCREEN.height/nbs*(i+2)-ren.get_height()/2))
        event = pygame.event.poll()
        if event.type == QUIT:
            pygame.quit()
            return
...
def MenuMode(screen):
    ...
    while 1:
        ...
        screen.blit(ren, (SCREEN.centerx-ren.get_width()/2, SCREEN.height/6*1-ren.get_height()/2))
        if score>0:
            ren = font.render("Score %d" % score, 1, Color("green"))
            screen.blit(ren, (SCREEN.centerx-ren.get_width()/2, SCREEN.height/6*2-ren.get_height()/2))
        ren = font.render("Press Space to start - h for high-scores - Esc to quit", 1, Color("white"))
        screen.blit(ren, (SCREEN.centerx-ren.get_width()/2, SCREEN.height/6*3-ren.get_height()/2))
        ren = font.render("Sound "+soundonoff+" (press s to toggle)", 1, Color("white"))
        screen.blit(ren, (SCREEN.centerx-ren.get_width()/2, SCREEN.height/6*4-ren.get_height()/2))
        ren = font.render("Language: "+CURLANG+" (press l to change)", 1, Color("white"))
        screen.blit(ren, (SCREEN.centerx-ren.get_width()/2, SCREEN.height/6*5-ren.get_height()/2))
        event = pygame.event.poll()
        if event.type == QUIT:
            pygame.quit()
            return
